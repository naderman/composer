...
class LocalRepoTransactionTest extends TestCase
{
    public function testTransactionGenerationAndSorting()
    {
        ...
        $lockRepo = new LockArrayRepository(array(
            clone $packageA,
            clone $packageAalias,
        ...
        $transaction = new LocalRepoTransaction($lockRepo, $localRepo);
        $this->checkTransactionOperations($transaction, $expectedOperations);
    }

    protected function checkTransactionOperations(LocalRepoTransaction $transaction, array $expected)
    {
        $result = array();
